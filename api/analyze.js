...
Motion data is PLAY-LEVEL: it means the offense used pre-snap motion on that snap, NOT that this player was the one in motion. It is a team-scheme signal observed on a player's targets. You may use it to support a scheme read alongside the play-caller notes. You must NEVER present it as a player-level motion split, never say a player "is better in motion," and never cite the percentage as a stat about the player himself. If you cannot make the point without implying player-level motion usage, leave it out.
