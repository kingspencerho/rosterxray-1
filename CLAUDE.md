...
`METRICS-PLAIN-ENGLISH.md` is the READING copy of the analysis reference — every metric
grouped by the question it answers rather than by stickiness, in Source Hierarchy order, with
the engineering metadata stripped. ⛔ **GENERATED by `scripts/build-plain-english.py`; never
hand-edit it.** Rebuild it in the same commit that changes §2, §3 or any §5 entry of the file
below — `scripts/build-plain-english.py --selftest` proves the round trip.
