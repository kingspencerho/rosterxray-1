...
// Next Gen Stats receiving: separation at the catch point, and intended air
// yards (aDOT, counted where the ball was AIMED whether or not it arrived).
//
// SEPARATION IS THE ONLY TALENT-IN-ISOLATION INPUT THIS APP CARRIES. Every
// other receiving number here measures what a coach handed a player — target
// share, WOPR, snap share are all opportunity. Separation measures whether he
// earned it, which is rank 3 in the Source Hierarchy and previously absent.
// Intended air yards is rank 2 and, at r=0.83, the stickiest player input
// measured anywhere in this project: it describes WHERE a receiver is deployed,
// and deployment persists while outcomes do not. Built by
...
// The cheapest available correction to the most common projection error:
// carrying a season forward on a player the calendar is working against.
// ⚠️ The bands are PRIORS, not measured in this repo — see the file's _meta.
// Built by scripts/build-career-arc.py. CONTEXT ONLY.
import CAREER_ARC from './grading/data/career_arc_2026.json';
// Per-team share of 2025 targets belonging to players no longer on the roster.
// Rank 1 in the Source Hierarchy is role/opportunity CHANGE, and this is the
// only file that measures it at the team level — Lens 1 requires projecting who
// absorbs a vacated share before ADP does, and nothing in the app could see the
...
const ADP_DATA = {
  ...
  "omar cooper": { adp: 153.4, pos: "WR", team: "NYJ" },
  "keenan allen": { adp: 153.4, pos: "WR", team: "IND" },  // added 2026-08-30: signed IND 1yr (reported Aug 17 2026); ADP from a live Underdog board  // ADP refresh 2026-08-16: 139.9 -> 153.4 (Underdog best ball, 0 drafts, live to 2026-08-16)
...
const ADP_SUPERFLEX = {
  ...
  "charlie kolar": { adp: 210.8, pos: "TE", team: "LAC" },  // read off the user's live Underdog superflex board Aug 23 2026
  ...
  "keenan allen": { adp: 233, pos: "WR", team: "IND" },
...
const RECENT_NEWS = {
  ...
  "tyreek hill": "FREE AGENT and undraftable in any 2026 best-ball format (updated Aug 28 2026). Miami released him in February after a dislocated left knee with multiple torn ligaments including the ACL, suffered in September 2025, and he has had two surgeries. In July he described having no power in that leg and being one day at a time. HIS AGENT HAS SAID PLAINLY THAT HE IS NOT EXPECTED TO SIGN BEFORE THE SEASON STARTS, with the stated plan being to join a contender later in the year and affect the playoff race; Rosenhaus points to running-hills footage as evidence of progress, which is an agent\'s framing rather than a medical clearance. THE BEST-BALL CONCLUSION IS UNCHANGED BY ANY OF IT: a player who signs mid-season to a team unknown cannot be assigned a W15-17 schedule, which is the only thing this format scores. Any table listing him on MIA is describing 2025.",
  ...
  "tucker kraft": "GB — BACK IN TEAM DRILLS and tracking for the Sept 13 opener vs MIN (updated Aug 28 2026). Activated off PUP Jul 31 2026 after tearing an ACL in Week 9 on Nov 2 2025, he has progressed from individual work and walkthroughs to full team periods, including the late-August joint practice against Arizona. Kraft says he does not expect a Week 1 pitch count. TWO THINGS TO HOLD TOGETHER ON HIS 2025 NUMBERS: he was TE3 in points per game and first among tight ends in points over expected per target, and that line covers Weeks 1-9 only, so his 86% snap share is a partial-season read on a role the full year never tested. The recovery pace supports paying the current price; the sample size is the honest caveat.",
  "malik nabers": "NYG — ON TRACK FOR WEEK 1, not yet confirmed (updated Aug 28 2026). He avoided the PUP list and took a full practice on camp Day 1 after ACL and meniscus surgery plus a second spring procedure to remove scar tissue; observers called the session outstanding. Nabers has said there is no target date and the Giants are evaluating his workload daily. The realistic downside is missing a game or two to open the season rather than an extended absence. His pre-injury usage is what justifies the price: a 28.7% target share and a 0.761 WOPR across his four 2025 games, higher than Jefferson or Chase, with 25% spike and 25% nuclear week rates on a small sample.",
  ...
  "zach charbonnet": "SEA — ON RESERVE/PUP, so the earliest he can play is Week 5 (updated Aug 28 2026). He tore his left ACL in the divisional round and did not have surgery until Feb 20 2026, and Seattle placed him on PUP before camp opened. THE TIMELINE HAS NARROWED AND IT IS NOT THE OPTIMISTIC ONE: the club has said a Week 1 return looks unlikely, Macdonald said as far back as the March league meetings that opening the season was improbable while committing to him playing at some point in 2026, and Jeremy Fowler reported a late-July checkpoint with progress described as good. Remaining on PUP into the regular season costs him a minimum of four games. Under a rule adopted for 2026 a reserve/PUP player can open a 21-day practice window after his team\'s second game, so he can be back at work well before he is eligible to play. This directly gates Jadarian Price\'s early-season workload.",
  ...
  "alec pierce": "IND — ACTIVATED off the active/PUP list on Thu Aug 27 2026, clearing him to practice. He had ankle surgery in March after an unsuccessful PRP injection and missed all of training camp; he now has two-plus weeks of practice before Week 1, which the Colts have said is the goal. THE AVAILABILITY QUESTION IS ANSWERED AND THE ROLE QUESTION IS NOT — he has taken no team reps this summer, so treat Week 1 usage as projected rather than established until he practices in full. Indianapolis paid him 4yr/$116M in March, which is the org commitment signal; the market moved his ADP from 53.8 to 90.0 while he was out, so the discount is still on the board.",
  ...
  "colbie young": "CIN — HE PRODUCED IN THE PRESEASON AND THE DEPTH CHART STILL SAYS IOSIVAS (updated Aug 29 2026). Fourth-round rookie WR competing with Andrei Iosivas for the WR3 job. THE EVIDENCE NOW RUNS BOTH WAYS. For Young: he led Cincinnati in the second preseason game with 3 catches for 48 yards and the team's first touchdown of the night, after a camp of contested-catch work off a 6-foot-3 frame, a ladder-climbing sideline grab and a red-zone score in the same practice, plus reporting that he has earned Joe Burrow's trust. For Iosivas: he started the Aug 13 opener vs Detroit and caught both his targets for 20 yards while Young took one snap on the second drive and played on after the starters left, and he then SAT OUT the Bears game entirely — resting with the starters is the treatment a penciled-in starter gets, and it is a stronger role signal than any box score a backup posts against backups. Iosivas holds the lead; his drops are the thing that could still cost it. WHY THE CEILING IS SHAPED THE WAY IT IS: the best WR tandem in football is ahead of both of them, so WR3 in Cincinnati is a low-target job in a very high-quality passing offense — a spike-week and red-zone dart, not a volume play, and worth a pick only at the very end of a board. FORMAT NOTE: CIN's W17 is BAL at home, the single best game environment on the board, which is where a cheap CIN pass catcher earns his roster spot. CIN playoff slate: W15 @CAR, W16 @IND, W17 vs BAL.",
  ...
  "charlie kolar": "LAC — the snap data says every-down tight end, not blocking specialist (updated Aug 23 2026). On the Chargers' two first-team drives in preseason Week 2 he played EVERY snap in 11 personnel, the package that puts one tight end on the field with three receivers. He splits 12 personnel with David Njoku and cedes 21 personnel to him, and he rested with the starters in the preseason opener while Oronde Gadsden worked with the backups. GRADE THE USAGE OVER THE DEPTH CHART, which lists him third. His 3yr/$24.3M deal — the largest ever for a blocking tight end — bought a top-6 PFF run blocker among TEs with 400+ snaps, and under Jim Harbaugh with OC Mike McDaniel that blocking is the reason he stays on the field for three downs. THE RECEIVING TOOLS ARE REAL AND COLLEGE-PROVEN: 168 catches, 2,181 yards and 23 TDs at Iowa State, all three school TE records, the only three-time All-American in program history. His 10-catch NFL career high came as the third option behind Mark Andrews and Isaiah Likely and describes Baltimore's pecking order rather than his ability. The open question is target share, not playing time — Njoku and Gadsden are both live and the offense runs. Per Lens 4 this is Week 2 preseason, so confirm against the Week 3 split. LAC playoff slate: W15 vs SF, W16 @MIA, W17 vs KC, with TE grading Good / Smash / Hard.",
  ...
  "james cook": "BUF — HEALTHY, FULL WORKLOAD, AND THE RECEIVING EXPANSION IS ON THE RECORD (updated Aug 28 2026). He played the whole first quarter of the preseason opener and carried five times for 21 yards including a 13-yard gain that set up a touchdown. New OC Pete Carmichael has said explicitly he wants Cook more involved as a pass catcher, and Buffalo is reported to be planning a larger receiving role for him in 2026. That is the thesis confirmed by the coach rather than inferred from his history: Carmichael fed Payton-tree backs for eighteen years, and the last play-caller to use Cook this way put him on a 63-target pace in 2023. Bell-cow usage in a top-6 implied offense behind a top-3 line, so last season\'s RB6 per-game reads as the floor.",
  ...
  "marshawn lloyd": "GB — THE JACOBS CONTINGENCY IS LIVE ON TWO FRONTS (updated Aug 29 2026). Josh Jacobs was charged Aug 27 2026 with two misdemeanors and is under an open NFL review the Packers have publicly said they are preparing for, and separately a groin injury kept Jacobs off the practice field for the better part of two weeks. Lloyd took the MAJORITY OF FIRST-TEAM SNAPS in that stretch and is the clear RB2. He came through his most productive training camp healthy — 'I feel like myself' after the final summer practice — and OC Adam Stenavich says his confidence in him has grown daily, calling the preseason a telling point for how much they can do with him. THE CAVEAT IS HIS RECORD, NOT HIS ROLE: one career NFL game, and all of last season lost to an injury on the first day of padded practices. Gutekunst has also said he may add a back if a suspension lands. GB W15 vs MIA and W16 @CHI are the usable playoff weeks; W17 vs HOU is a wall.",
...
const VERDICTS = {
  ...
  "mike washington": { verdict: "TARGET", date: "2026-08-23", reason: "The contingency is live: Jeanty left an Aug 23 practice unable to bear weight on his right knee and Washington took the rest of the first-team reps, having already been alternating starting reps before it. Kubiak deploys even two-back splits regardless of talent differential (Walker/Charbonnet in SEA is the precedent), so he had standalone volume even at full health. Severity of the Jeanty injury is unannounced, so size the bet on role, not on a timeline. LV W16 vs TEN (soft) and W17 @ARI (soft) are two viable playoff weeks. Combine fraud and fumble history are real concerns but irrelevant at this price in a Kubiak scheme that manufactures opportunities.", confidence: "SPECULATIVE" },
  ...
  "jordyn tyson": { verdict: "fade", date: "2026-08-23", reason: "Hamstring injury with a reported absence of about two months, which puts him in range of opening the season on IR. The 8th-overall capital and the WR2 role in Shough's offense are intact and the NO W15-17 slate is still soft, so the playoff-window case survives — but the absence lands on the W1-14 qualifying round, and an IR start would cost more than a discount is worth in any format that scores those weeks.", confidence: "MEDIUM-HIGH" },
...
const SITUATIONS = {
  ...
  "malachi fields": { verdict: "DART", trend: "rising", trendNote: "NYG third-round rookie (74th overall) the Giants TRADED UP to take, which is the draft-capital signal (updated Aug 29 2026). 6-4, 222, primarily an X receiver projecting to perimeter snaps as a rookie. He has become a favorite target of Jaxson Dart through training camp and has climbed the depth chart steadily. THE STACK LOGIC IS THE POINT: he is a Dart correlation piece at ADP 189.7, so a late pick buys stack depth rather than a standalone bet. Malik Nabers is the alpha and is tracking to play Week 1, so Fields is the WR2/WR3 in a passing game that will be trailing often — and per the blowout-risk archetype split, a big-bodied perimeter profile is the one that gets CAPPED in negative scripts, not the one that gets a garbage-time floor. Contested-catch and red-zone work is the realistic route to weekly relevance.", situationFlags: ["target_vacuum"], riskFlags: ["depth_chart_competition"] },
  "ted hurst": { verdict: "DART", trend: "stable", trendNote: "TB third-round rookie (84th overall), 6-4, and the organization has drawn the Mike Evans comparison on the strength of his ability to play above the rim (updated Aug 29 2026). HIS ROLE IS EXPLICITLY HEALTH-CONTINGENT: how early and how often Baker Mayfield throws to him depends on the status of Emeka Egbuka, Jalen McMillan and Tez Johnson, all of whom have missed time this summer. That is a genuine contingency rather than a projection, because the injuries are current. Standalone expectation is a WR4 role; the ceiling requires one of the three ahead of him to miss real time. Free at ADP 199.3, and TB W17 vs LAR is a viable playoff environment.", situationFlags: ["role_dependent"], riskFlags: ["depth_chart_competition"] },
  "zachariah branch": { verdict: "dart", trend: "stable", trendNote: "ATL third-round rookie (79th overall) with the WR4 ROLE LOCKED IN and a path to move up as the season goes (updated Aug 29 2026). Atlanta is deliberately OPENING THE ROUTE TREE for him — he was used mainly as a gadget player at Georgia and the staff is expanding him into a full receiver role, which per Lens 4 is a bullish signal rather than generic praise. THE CAVEAT IS THE QB: Atlanta has an unsettled competition with Penix having closed the gap on Tua, so the passing game a WR4 inherits is not projectable. Speed is the traits case and the draft capital backs it, but WR4 on a team with an unresolved quarterback is a depth-chart bet, not a target bet. ADP 198.9 prices that correctly.", situationFlags: ["scheme_fit"], riskFlags: ["depth_chart_competition"] },
  "jayden higgins": { verdict: "fade", trend: "falling", trendNote: "OUT FOR THE 2026 SEASON with a torn ACL (updated Aug 23 2026). He was set to start opposite Nico Collins in Houston, so this vacates a full-time outside role rather than a rotational one. THE TARGETS GO TO JAYLIN NOEL, XAVIER HUTCHINSON AND TANK DELL, and the WR2 job behind Collins is now an open competition among the three. Any depth-chart read of this receiver room written before this date needs re-sourcing. Rostering him costs a bench slot for the entire tournament in any format, and there is no contingency angle, since the player himself is the vacancy.", situationFlags: [], riskFlags: ["injury_history"] },
  "charlie kolar": { verdict: "DART", trend: "rising", trendNote: "LAC'S EVERY-DOWN TIGHT END, and preseason Week 3 confirmed it (updated Aug 29 2026). He was the ONLY Chargers tight end to play every snap with the starters, and he ran routes on 4 of 5 Justin Herbert dropbacks across 6 of 7 first-team snaps. Route participation is the prerequisite for a receiving role, and he now has it across three straight exhibitions. In Week 2 he took every snap in 11 personnel; he and David Njoku share the field in 12 and Njoku out-snaps him in 21. THE DEPTH CHART SAYS TE3 AND THE SNAPS SAY OTHERWISE — grade the usage. He is the best blocker in the room on a 3yr/$24.3M deal (highest-paid blocking TE in NFL history), 6th in PFF run block among TEs with 400+ snaps, and that is what keeps him on the field for three downs rather than what limits him to two. THE COLLEGE PROFILE IS A RECEIVER'S: 168-2181-23 at Iowa State, every school TE record, the only three-time All-American in program history and three straight First Team All-Big 12 seasons. His 10-catch NFL career high was set as the third option behind Mark Andrews and Isaiah Likely, so it measures Baltimore's pecking order rather than his hands. The remaining open question is TARGET SHARE, not snaps: Njoku and Gadsden are live and Harbaugh is run-heavy. Per Lens 4 this is now past Week 3, so snap share is the primary signal and it overrides the depth chart. Free at ADP, and LAC grades Good / Smash / Hard at TE across W15-17.", situationFlags: ["breakout_profile", "scheme_fit"], riskFlags: ["depth_chart_competition"] },
  ...
  "james cook": { verdict: "TARGET", trend: "rising", trendNote: "BELL-COW WITH A CONFIRMED RECEIVING EXPANSION (updated Aug 28 2026). Healthy through camp; he played the full first quarter of the preseason opener with five carries for 21 yards. OC Pete Carmichael has stated on the record that he wants Cook more involved in the passing game, and Buffalo is reported to be planning a larger receiving role — the ceiling case is now coach-confirmed rather than inferred. Carmichael fed Payton-tree backs for eighteen years, and the last play-caller to use Cook this way put him on a 63-target pace in 2023. Top-6 implied offense, top-3 offensive line, and last year\'s RB6 per-game is the floor.", situationFlags: ["scheme_fit"], riskFlags: [] },
  ...
  "ashton jeanty": { verdict: "TARGET", trend: "stable", trendNote: "SPRAINED ANKLE, believed short-term (updated Aug 28 2026). He dove for a low throw from Kirk Cousins in a team period, landed awkwardly and left practice. Reporting has it as an ankle sprain with no long-term damage; Las Vegas has given NO RETURN TIMELINE, and ankle sprains run one to six weeks depending on grade. So Week 1 is live but unconfirmed and the ceiling case is intact. THE ACTIONABLE PART IS THE CONTINGENCY: rookie fourth-rounder Mike Washington Jr. is the named beneficiary and would lead the backfield for any games Jeanty misses, at an ADP near 192.", situationFlags: ["scheme_fit"], riskFlags: ["injury_history"] },
  ...
  "alec pierce": { verdict: "hold", trend: "rising", trendNote: "ACTIVATED off active/PUP Aug 27 2026 with two-plus weeks of practice before Week 1 (March ankle surgery, no camp reps). Availability is resolved; the role is not, because he has taken no team reps this summer — grade Week 1 usage as projected. The 4yr/$116M deal signed in March is the commitment signal, and ADP moved 53.8 -> 90.0 during the absence, so the discount survives the good news.", situationFlags: ["role_dependent"], riskFlags: ["injury_history"] },
  ...
  "colbie young": { verdict: "DART", trend: "rising", trendNote: "PRODUCING, BUT THE JOB HAS NOT MOVED (updated Aug 29 2026). Fourth-round CIN rookie competing with Andrei Iosivas for WR3. Young led Cincinnati in the second preseason game — 3 catches, 48 yards and the first touchdown of the night — on top of a camp of contested-catch work off a 6-3 frame and reporting that he has earned Burrow's trust. THE ROLE SIGNAL POINTS THE OTHER WAY: Iosivas started the Aug 13 opener and caught both his targets, then sat out the Bears game entirely, which is how a penciled-in starter is handled and outweighs a backup's box score against backups. Iosivas leads; his drops are what could still cost him. DART rather than TARGET on purpose — WR3 behind the best receiver tandem in football is a low-target role, so this is a spike-week and red-zone profile, not a volume one. The reason to hold him is CIN's W17 vs BAL, the best game environment on the board.", situationFlags: ["breakout_profile"], riskFlags: ["creeping_committee", "role_dependent"] },
  ...
  "jacoby brissett": { verdict: "hold", trend: "stable", trendNote: "THE WEEK 1 JOB IS SETTLED (updated Aug 28 2026). He signed a reworked deal in late Jul 2026 — $15.5M base with up to $21M in incentives, described as clear-cut starter money — reported on time and is practising. Reporting as of the Aug 6 Hall of Fame game is explicit: the plan is for Brissett to start Week 1 against the Chargers. Beck is a 3rd-round rookie whom HC LaFleur wants to sit ('it benefits anyone in any position to be able to sit back'), so the leash is long. Low ceiling on a rebuilding offense, but he is the confirmed starter, not a fade.", situationFlags: [], riskFlags: [] },
  // === ROLE CEILING FLAGS ===
  ...
  "josh jacobs": { verdict: "fade", trend: "falling", trendNote: "CHARGED AUG 27 2026, UNDER NFL REVIEW, AND HURT (updated Aug 29 2026). The Brown County DA filed two misdemeanors out of the May 26 arrest — battery and criminal damage to property — with an initial court appearance set for Nov 17 2026. THE DETAIL THAT MATTERS IS WHAT THE FILING LEAVES OUT: it is a reduction from five original counts and carries NO domestic-violence designation, which weakens the case for the CBA's six-game domestic-abuse baseline. That is the favourable half. The unfavourable half is that the personal conduct policy runs independently of the criminal charge, the league review is still open with no decision announced, and GM Brian Gutekunst has said on the record that Green Bay has been preparing for a suspension and may add backfield depth. SEPARATELY AND COMPOUNDING IT, a groin injury has kept him out of practice for the better part of two weeks, and MarShawn Lloyd took the majority of first-team snaps while he was down. TWO INDEPENDENT AVAILABILITY RISKS ON ONE PLAYER at an ADP near 41, both unresolved before draft day, and a conduct ban lands at the front of the season with no warning window.", situationFlags: [], riskFlags: ["contract_year", "injury_history"] },
  ...
  "marshawn lloyd": { verdict: "DART", trend: "rising", trendNote: "THE GREEN BAY CONTINGENCY, AND IT IS CONVERGING FROM TWO DIRECTIONS AT ONCE (updated Aug 29 2026). Josh Jacobs was charged Aug 27 with two misdemeanors and remains under an open NFL review that GM Brian Gutekunst says the club is preparing for, AND a groin injury has kept Jacobs out of practice for most of two weeks. Lloyd took the MAJORITY OF FIRST-TEAM SNAPS while Jacobs was down and projects as the clear RB2. HEALTH IS THE WHOLE THESIS AND IT IS FINALLY CLEAN: he came through his most productive camp intact and said he feels like himself after the final summer practice, and OC Adam Stenavich describes his confidence growing by the day. THE HONEST COUNTERWEIGHT IS THE TRACK RECORD: one career NFL game, and a full season lost to an injury suffered on the first day of padded practices. So this is a durability bet, not a role bet — the path is real and the body has not yet proven it can hold a workload. Gutekunst has also left the door open to adding a back, which caps how clean the succession is. Draft him as the Jacobs hedge and as a standalone dart at a price the market is already moving: ADP has run from 215.5 to the low 140s since mid-August.", situationFlags: ["breakout_profile", "role_dependent"], riskFlags: ["injury_history"] },
...
const ADP_YAHOO = {
  ...
  "keenan allen": { adp: 262.0, pos: "WR", team: "IND" },
...
const getNgsRec = (name) => lookupPlayer(NGS_RECEIVING.players, name);
...
const parseNewsDate = (text, nowTs = Date.now()) => {
  if (!text) return null;
  // BOTH passes always run, and the LATEST timestamp wins across them.
  // Stopping at the first pass would let an OLD day-precision date beat a NEWER
  // month-only one — Nabers' note carries "Sep 28 2025" (the ACL tear) and
  // "Aug 2026 camp", and the tear was being stamped as his currency. Precision
  // only breaks a tie, which it does for free: a month-only date resolves to the
  // 1st, so a same-month day form is always the larger timestamp.
  let best = null;
  for (const [re, hasDay] of [[NEWS_DATE_DAY, true], [NEWS_DATE_MONTH, false]]) {
    ...
    while ((m = re.exec(text))) {
      ...
      // A FUTURE DATE IS NEVER THE NOTE'S CURRENCY. A note cannot have been
      // written after today, so a forward-looking date in the prose — a
      // scheduled court appearance, a return window, a contract deadline — is
      // describing something the note is ABOUT, not when it was written.
      // Josh Jacobs' entry cites a Nov 17 2026 court date and was stamping
      // itself "-80d ago" until this guard existed.
...
const CARD_GLOSSARY = {
  ...
  ngs_iay: {
    term: "Intended air yards",
    what: "How far downfield the ball is travelling when it is thrown at him, averaged over every target — counted where it was aimed, whether or not he caught it.",
    how: "The stickiest player number on this card. It describes WHERE the offence uses him, and deployment carries from season to season even when production does not. Two receivers with the same target count are different assets if one runs slants and the other runs digs.",
  },
  ngs_sep: {
    term: "Separation",
    what: "Yards of space between him and the nearest defender at the moment the pass arrives, from the tracking chip in the ball.",
    how: "The only number here that measures the player rather than his opportunity. Everything else says what a coach gave him; this says whether he is earning it. Big separation on modest volume is the shape a breakout has before the targets show up.",
  },
  _arc: {
    term: "Career arc",
...
// NGS carries its OWN population (40+ targets in 2025) and its own gate, which
// is not the gate CARD_PERCENTILES uses. Ranking one against the other would
// print a percentile whose stated population is wrong, so the two never share a
// table and the NGS section prints its own gate.
const NGS_PERCENTILES = (() => {
  const out = {};
  for (const r of Object.values(NGS_RECEIVING.players || {})) {
    const b = (out[r.pos] ||= { sep: [], iay: [] });
    if (typeof r.sep === "number") b.sep.push(r.sep);
    if (typeof r.iay === "number") b.iay.push(r.iay);
  }
  for (const b of Object.values(out)) { b.sep.sort((a, c) => a - c); b.iay.sort((a, c) => a - c); }
  return out;
})();
...
// The band a player's age falls in, from CAREER_ARC._meta. The bands are
// published priors rather than a finding of this repo, so the card labels them
// that way wherever they render.
const careerPhaseLabel = (arc) => {
  const band = CAREER_ARC._meta.bands?.[arc?.pos];
  if (!arc || !band) return null;
  const phase = arc.phase;
  const text = phase === "rising" ? `ascending — ${arc.pos} production typically peaks around ${band.peak}`
    : phase === "peak" ? `in the peak band for a ${arc.pos} (about ${band.rising}-${band.decline})`
    : `past the ${band.decline} mark where ${arc.pos} production typically starts to fall`;
  return { phase, text, note: band.note, peak: band.peak, decline: band.decline };
};
...
const buildPlayerCard = (name, pos, team, nowTs = Date.now(), format = "standard") => {
  ...
  // DEPLOYMENT — separation and intended air yards, from Next Gen Stats.
  // Deliberately its OWN section rather than extra rows inside Opportunity:
  // different source, different population, different printed gate. Merging
  // them would put a percentile under a population label that does not describe
  // it, which is the vintage trap wearing a new costume.
  const ngs = (pos === "WR" || pos === "TE") ? getNgsRec(name) : null;
  if (ngs) {
    const st = NGS_RECEIVING._meta.stability;
    const med = NGS_RECEIVING._meta.medians[pos];
    card.deployment.push({
      key: "ngs_iay", label: "Intended air yards", r: st.avg_intended_air_yards.r, tier: "anchor",
      value: `${ngs.iay.toFixed(1)} yds`, pct: ngsPercentile(pos, "iay", ngs.iay),
    });
    card.deployment.push({
      key: "ngs_sep", label: "Separation", r: st.avg_separation.r, tier: "reliable",
      value: `${ngs.sep.toFixed(2)} yds`, pct: ngsPercentile(pos, "sep", ngs.sep),
    });
    // The median goes in the section NOTE, never in a row's `caution`. On this
    // card `caution` means "this number disagrees with another one above it" and
    // renders with a warning mark; a reference value is not a conflict, and
    // dressing one as a warning devalues the mark everywhere it appears.
    card.deploymentSepMedian = med;
    card.deploymentTargets = ngs.tgt;
    card.deploymentTeam = ngs.team;
  }

  // CAREER ARC. Age is a fact; the band around it is a published prior, and the
  // card says so rather than dressing a borrowed curve as a measurement.
  const arc = getCareerArc(name);
  if (arc) {
    const phase = careerPhaseLabel(arc);
    card.arc = {
      age: arc.age, exp: arc.exp, draft: arc.draft, team: arc.team,
      phase: arc.phase, text: phase?.text || null, note: phase?.note || null,
      asOf: CAREER_ARC._meta.as_of,
    };
  }

  // TEAM TARGET TURNOVER. Team-level and stated as such: it locates a vacancy,
  // it does not name the player who fills it.
  const vac = getVacated(team);
  if (vac && typeof vac.vacated_pct === "number") {
    card.vacated = {
      team, pct: vac.vacated_pct,
    ...
    >
      ...
      >
        ...
        {/* CAREER ARC AND TEAM TURNOVER SIT ABOVE THE no-data BRANCH ON PURPOSE.
            ...
            ONLY useful information on the card. Rendering them inside the
        ...
        {card.arc && (
          <CardSection
            title="Career arc"
            accent={CARD_ACCENTS.arc}
            collapsible
            hint={`age ${card.arc.age} · ${card.arc.phase}`}
            note="Age is measured. The band around it is a published aging prior, not a finding of this app — weigh it, do not quote it.">
            <div style={{ display: "flex", gap: "18px", flexWrap: "wrap", fontSize: "12px", marginBottom: "7px" }}>
              {[["Age", card.arc.age], ["Season", `#${card.arc.exp + 1}`], ["Draft", card.arc.draft ? `pick ${card.arc.draft}` : "undrafted"]].map(([k, v]) => (
                <div key={k}>
                  <div style={{ fontSize: "10px", color: "var(--text-dim)", letterSpacing: "0.06em", textTransform: "uppercase" }}>{k}</div>
                  <div style={{ color: "var(--text-primary)", fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{v}</div>
                </div>
              ))}
            </div>
            {card.arc.text && (
              <div style={{ fontSize: "11px", color: "var(--text-secondary)", lineHeight: 1.5 }}>{card.arc.text}.</div>
            )}
            {card.arc.note && (
        ...
        {card.vacated && (
          <CardSection
            ...
            accent={CARD_ACCENTS.vacated}
            collapsible
            hint={`${Math.round(card.vacated.pct)}% of ${card.vacated.team} targets gone`}
            note="Share of this team's 2025 targets belonging to players no longer on the roster. It locates the opening; who inherits it is still a judgement.">
            <div style={{ fontSize: "22px", fontFamily: "var(--font-display)", color: "var(--text-primary)", lineHeight: 1.1 }}>
              {Math.round(card.vacated.pct)}%
              <span style={{ fontFamily: "inherit", fontSize: "11px", color: "var(--text-dim)", marginLeft: "8px", letterSpacing: 0 }}>
                of {card.vacated.team}'s measured 2025 targets
              </span>
            </div>
            {card.vacated.gone.length > 0 ? card.vacated.gone.map((g, i) => (
              <div key={i} style={{ display: "flex", alignItems: "baseline", gap: "8px", fontSize: "11px", padding: "4px 0", borderTop: "1px solid var(--bg-raised)" }}>
                <span style={{ color: "var(--text-primary)", fontWeight: 600, textTransform: "capitalize" }}>{g.name}</span>
                <span style={{ color: "var(--text-dim)" }}>{g.pos} · {g.status}</span>
                <span style={{ marginLeft: "auto", color: "var(--text-secondary)", fontVariantNumeric: "tabular-nums" }}>{Math.round(g.tgt_sh * 100)}%</span>
              </div>
            )) : (
    ...
    try {
      ...
      const metricsContext = (result.valid || [])
        .map(p => {
          ...
          // TARGETS PER GAME AND AIR YARDS SHARE LEAD THE RECEIVING LINE, because
          // measured year over year they are the two most repeatable numbers in
          // it (0.774 and 0.780) and both were previously loaded and unprinted —
          // the model saw target share, WOPR and snap share and never saw the
          // pair that anchors them. Targets/gm is the raw volume nothing else
          // survives being low; air yards share is where on the field that volume
          // is aimed, which is what separates an alpha from a high-volume slot
          // player at the same target count.
          //
          // Targets/gm is computed over GAMES PLAYED, never a 17-game
          // denominator — a full-season denominator understates every player who
          // missed time, which is the bug fixed here Jul 16 2026.
          if (p.pos === "WR" || p.pos === "TE") bits.push(`${m.gp ? (m.tgt / m.gp).toFixed(1) : "?"} tgt/gm${m.ay_sh != null ? `, ${Math.round(m.ay_sh * 100)}% air yards share` : ""}, ${Math.round(m.tgt_sh * 100)}% tgt share (games played), WOPR ${m.wopr}${m.snap_sh != null ? `, ${Math.round(m.snap_sh * 100)}% snap share (route-participation proxy)` : ""}`);
          // Backs get targets/gm too — the receiving tier is a stack qualifier and
          // a garbage-time exemption, so the volume behind it belongs in the line.
          // Air yards share is deliberately WITHHELD here: it is r=0.26 for backs
          // and discriminates nothing, so printing it would invite the model to
          // read a number that carries no information.
          if (p.pos === "RB") bits.push(`${m.hvt_pg} HVT/gm${m.gp ? `, ${(m.tgt / m.gp).toFixed(1)} tgt/gm` : ""}${m.expl_pct != null ? `, ${Math.round(m.expl_pct * 100)}% explosive carries` : ""}`);
      ...
      // === NGS DEPLOYMENT CONTEXT ===
      // The two receiving numbers the prompt never carried, and the two that
      // most deserve to be there. Intended air yards is the stickiest player
      // input measured in this project (r=0.83) and separation is the only
      // talent-in-isolation figure the app holds (r=0.66) — everything else in
      // the receiving block is opportunity handed down by a coach.
      const deploymentContext = (result.valid || [])
        .map(p => {
          if (p.pos !== "WR" && p.pos !== "TE") return null;
          const n = getNgsRec(p.name);
          if (!n) return null;
          const med = NGS_RECEIVING._meta.medians[p.pos];
          const sepRead = n.sep >= med + 0.4 ? " — gets open comfortably for the position"
            : n.sep <= med - 0.4 ? " — wins less separation than the position median"
            : "";
          const moved = n.team && p.team && n.team !== p.team ? ` [2025 with ${n.team}, now ${p.team}]` : "";
          return `${p.name}: ${n.iay.toFixed(1)} intended air yards (aDOT — where he is deployed), ${n.sep.toFixed(2)} yds separation vs ${p.pos} median ${med}${sepRead}, on ${n.tgt} targets${moved}`;
        })
        .filter(Boolean)
        .join("\n");

      // === CAREER ARC CONTEXT ===
      // Only the tails. A player sitting inside his position's peak band tells
      // the model nothing his other numbers did not, so silence here means the
      // calendar is neutral rather than that the data is missing — stated in
      // the prompt header, same rule the trajectory block follows.
      const arcContext = (result.valid || [])
        .map(p => {
          const a = getCareerArc(p.name);
          if (!a || a.phase === "peak") return null;
          const band = CAREER_ARC._meta.bands[a.pos];
          if (!band) return null;
          const read = a.phase === "decline"
            ? `past ${band.decline}, where ${a.pos} production typically starts to fall`
            : `still ascending — ${a.pos} production typically peaks around ${band.peak}`;
          return `${p.name}: age ${a.age}, season #${a.exp + 1}, ${a.draft ? `drafted ${a.draft} overall` : "undrafted"} — ${read}`;
        })
        .filter(Boolean)
        .join("\n");

      // === VACATED TARGET CONTEXT ===
      // Team-level, deduped, and only the teams this roster actually touches.
      // Rank 1 in the Source Hierarchy and the app had no view of it at all.
      const vacatedContext = (() => {
        const teams = [...new Set((result.valid || []).map(p => p.team).filter(Boolean))];
        return teams
          .map(t => {
            const v = getVacated(t);
            // vacated_pct is stored in PERCENT units (46.6), not as a fraction.
            if (!v || !(v.vacated_pct > 20)) return null;
            const gone = (v.gone || []).slice(0, 4)
              .map(g => `${titleCaseName(g.name)} ${g.pos} ${Math.round(g.tgt_sh * 100)}% (${g.status})`)
              .join(", ");
            return `${t}: ${Math.round(v.vacated_pct)}% of measured 2025 targets have left${gone ? ` — ${gone}` : ""}`;
          })
          .filter(Boolean)
